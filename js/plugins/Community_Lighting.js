...
Community.Lighting.parameters = PluginManager.parameters(
  Community.Lighting.name
);
...
(function ($$) {
  ...
  let useSmootherLights =
    eval(String(parameters['Use smoother lights'])) || false;
  let light_event_required = eval(parameters['Light event required']) || false;
  let shift_lights_with_events =
    eval(String(parameters['Shift lights with events'])) || false;
  let player_radius = Number(parameters['Player radius']) || 0;
  let reset_each_map = eval(String(parameters['Reset Lights'])) || false;
  let noteTagKey =
    parameters['Note Tag Key'] !== '' ? parameters['Note Tag Key'] : false;
  ...
  let dayNightNoAutoshadow =
    eval(parameters['No Autoshadow During Night']) || false;
  ...
  let dayNightList = (function (dayNight, nightHours) {
    ...
    try {
      ...
      console.log(
        'CommunityLighting: Night Hours and/or DayNight Colors contain invalid JSON data - cannot parse.'
      );
      result = new Array(24)
        .fill(undefined)
        .map((x) => (x = { color: '#000000', isNight: false }));
    }
    return result;
  ...
  $$.validateColor = function (color, defaultColor = '#ffffff') {
    ...
    if (!isValid)
      console.log('Community_Lighting_MZ - Invalid Color: ' + color);
  ...
  $$.saveTime = function (hh, mm, ss = null) {
    ...
    if (dayNightSaveMinutes > 0)
      $gameVariables.setValue(dayNightSaveMinutes, mm);
    if (dayNightSaveSeconds > 0 && ss !== null)
      $gameVariables.setValue(dayNightSaveSeconds, ss);
    if (dayNightSaveNight > 0 && dayNightList[hh] instanceof Object)
      $gameSwitches.setValue(dayNightSaveNight, dayNightList[hh].isNight);
    if (
      dayNightNoAutoshadow &&
      dayNightList[hh] instanceof Object &&
      dayNightList[hh].isNight !== hideAutoShadow
    ) {
  ...
  $$.isNight = function () {
    ...
    return dayNightList[hour] instanceof Object
      ? dayNightList[hour].isNight
      : false;
  };
  $$.hours = function () {
  ...
  $$.minutes = function () {
    return Math.floor(
      $gameVariables.GetDaynightTimer() / $gameVariables.GetDaynightSpeed()
    );
  };
  $$.seconds = function () {
    let speed = $gameVariables.GetDaynightSpeed();
    let value = Math.floor(
      $gameVariables.GetDaynightTimer() - speed * $$.minutes()
    );
    ...
    if (this._clType === 'light' || this._clType === 'fire') {
      ...
      for (let x of tagData) {
        if (!isNaN(+x) && this._clRadius === undefined) this._clRadius = +x;
          ...
          this._clSwitch = x;
        else if (x[0] === 'd' && this._clDirection === undefined)
          this._clDirection = +x.substr(1, x.length);
        else if (x[0] === 'x' && this._clXOffset === undefined)
          this._clXOffset = +x.substr(1, x.length);
        else if (x[0] === 'y' && this._clYOffset === undefined)
          this._clYOffset = +x.substr(1, x.length);
      ...
      this._clRadius = 1;
      for (let x of tagData) {
        if (!isNaN(+x) && this._clBeamLength === undefined)
          this._clBeamLength = +x;
        else if (!isNaN(+x) && this._clBeamWidth === undefined)
          this._clBeamWidth = +x;
        else if (x[0] === 'l' && this._clBeamLength === undefined)
          this._clBeamLength = this._clBeamLength = +x.substr(1, x.length);
        else if (x[0] === 'w' && this._clBeamWidth === undefined)
          this._clBeamWidth = this._clBeamWidth = +x.substr(1, x.length);
        else if (x === 'cycle' && this._clColor === undefined)
          this._clCycle = [];
        else if (this._clCycle && !needsCycleDuration && x[0] === '#') {
          this._clCycle.push({ color: $$.validateColor(x), duration: 1 });
          ...
          this._clCycle[this._clCycle.length - 1].duration = +x || 1;
          needsCycleDuration = false;
        } else if (x[0] === '#' && this._clBeamColor === undefined)
          this._clColor = $$.validateColor(x);
        else if (!isNaN(+x) && this._clOnOff === undefined) this._clOnOff = +x;
        else if (!isNaN(+x) && this._clFlashlightDirection === undefined)
          this._clFlashlightDirection = +x;
        else if (x === 'on' && this._clOnOff === undefined) this._clOnOff = 1;
        else if (x === 'off' && this._clOnOff === undefined) this._clOnOff = 0;
        else if ((x === 'night' || x === 'day') && this._clSwitch === undefined)
          this._clSwitch = x;
        else if (x[0] === 'd' && this._clFlashlightDirection === undefined)
          this._clFlashlightDirection = +x.substr(1, x.length);
        else if (x[0] === 'x' && this._clXOffset === undefined)
          this._clXOffset = +x.substr(1, x.length);
        else if (x[0] === 'y' && this._clYOffset === undefined)
          this._clYOffset = +x.substr(1, x.length);
  ...
  Game_Event.prototype.getLightEnabled = function () {
    ...
    if (this._clSwitch === undefined) {
      ...
      (result = this._clSwitch === 'night' && $$.isNight()) ||
   ...
   * @param {String[]} args
   */
  Game_Interpreter.prototype.communityLighting_Commands = function (
    command,
    args
  ) {
    ...
    const allCommands = {
      tileblock: 'tileType',
      regionblock: 'tileType',
      tilelight: 'tileType',
      regionlight: 'tileType',
      tilefire: 'tileType',
      regionfire: 'tileType',
      tileglow: 'tileType',
      regionglow: 'tileType',
  ...
  Game_Interpreter.prototype.fire = function (command, args) {
    if (args.contains('radius') || args.contains('radiusgrow'))
      $gameVariables.SetFire(true);
    if (
      args[0] === 'deactivate' ||
      (args[0].toLowerCase() === 'off' && args.length == 1)
    ) {
  ...
  Game_Interpreter.prototype.determineBattleTint = function (tintColor) {
    ...
    if (color < 300 && red < 100 && green < 100 && blue < 100) {
      // Check for NaN values or too dark colors
  ...
  Lightmask.prototype._updateMask = function () {
    ...
    if (daynightspeed > 0 && daynightspeed < 5000 && brightnessOverTime) {
      ...
      if (seconds > oldseconds) {
        ...
        let daynightseconds = Math.floor(
    ...
    for (let i = 0, len = eventObjId.length; i < len; i++) {
      ...
      if (lightsOnRadius > 0) {
        let distanceApart = Math.round(
          Community.Lighting.distance(
            $gamePlayer.x,
            $gamePlayer.y,
            cur._realX,
            cur._realY
   ...
   * @param {String} color2
   */
  CanvasGradient.prototype.addTransparentColorStops = function (
    brightness,
    color1,
    color2
  ) {
  ...
  BattleManager.setup = function (troopId, canEscape, canLose) {
    ...
    if (
      ...
      // If we went there from a map...
      if ($gameVariables.GetScriptActive() === true) {
        // If the script is active...
        if (options_lighting_on && lightInBattle) {
          // If configuration autorise using lighting effects
          if (eventObjId.length > 0) {
            // If there is lightsource on this map...
    ...
    Community_Lighting_BattleManager_setup.call(
      this,
      troopId,
      canEscape,
      canLose
  ...
  let Community_Lighting_Spriteset_Battle_createLowerLayer =
  ...
  let Community_Lighting_Spriteset_Battle_createBattleback =
  ...
  Spriteset_Battle.prototype.createBattleLightmask = function () {
    if ($gameVariables.GetScriptActive()) {
      // If the script is active
      if (lightInBattle) {
        // If is active during battles.
  ...
  BattleLightmask.prototype.initialize = function () {
    ...
    $gameTemp._BattleTint = $$.daynightset
      ? $gameVariables.GetTintByTime()
      : $gameTemp._MapTint;
    this._maskBitmap.FillRect(
      -lightMaskPadding,
      0,
  ...
  let Window_Options_addGeneralOptions =
    ...
    mapNote.forEach((note) => {
      ...
      if (mapnote) {
        ...
        if (/^daynight/i.test(mapnote)) {
          ...
          if (brightness) {
            $$.defaultBrightness =
              Math.max(0, Math.min(Number(brightness[0], 100))) / 100;
          }
...
Game_Variables.prototype.GetDaynightColorArray = function () {
  let result =
    this._Community_Lighting_DayNightColorArray ||
    Community.Lighting.getDayNightList();
  if (!result) {
    result = [
      '#000000',
      '#000000',
      ...
      '#000000',
      '#000000',
    ].map((x) => (x = { color: x, isNight: false }));
    this._Community_Lighting_DayNightColorArray = result;
  }
  if (!this._Community_Lighting_DayNightColorArray)
    this.SetDaynightColorArray(result);
  return result;
};
...
Game_Variables.prototype.GetDaynightSpeed = function () {
  ...
  var defaultSpeed = Number(
    Community.Lighting.parameters['Daynight Initial Speed']
  );
...
Game_Variables.prototype.GetDaynightCycle = function () {
  if (this._Community_Lighting_DaynightCycle !== undefined)
    return this._Community_Lighting_DaynightCycle;
...
Window_TimeOfDay.prototype.update = function () {
  ...
  if (this.visible) {
    ...
    this.drawTextEx(
      time,
      this.contents.width - textWidth - this.textPadding(),
      0
    );
  }
};
Community.Lighting.Scene_Map_createAllWindows =
...
Community.Lighting.Spriteset_Map_prototype_createLowerLayer =
