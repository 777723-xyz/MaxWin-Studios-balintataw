...
TSR.TextColorAddOn.getColorObj = function (textbox) {
  // Robust ES5 parser for the "Text Color Object" note param.
  // Avoids String.prototype.includes and coerces to strings.
  var obj = {};
  if (textbox == null) return obj;

  // Ensure we are working with a plain string
  var raw = String(textbox);
  // Split on commas (keeps the plugin's original behavior)
  var parts = raw.split(',');

  for (var i = 0; i < parts.length; i++) {
    var chunk = String(parts[i]);

    // Remove single newline inside the chunk:  "A\nB" -> "AB"
    var nlPos = chunk.indexOf('\n');
    if (nlPos !== -1) {
      chunk = chunk.slice(0, nlPos) + chunk.slice(nlPos + 1);
    }

    // Expect "code: value"
    var colon = chunk.indexOf(':');
    if (colon === -1) continue; // skip malformed entries

    var prop = String(chunk.slice(0, colon)).trim();
    var val = String(chunk.slice(colon + 1));

    // Handle rgba( r g b a ) -> rgba(r,g,b,a) (the plugin used 3 successive replaces)
    if (val.indexOf('rgba') !== -1) {
      // normalize multiple spaces to one comma
      // e.g. "rgba(200 180 0 0.5)" -> "rgba(200,180,0,0.5)"
      val = val.replace(
        /rgba\s*\(\s*([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)\s*\)/i,
        function (_m, r, g, b, a) {
          return 'rgba(' + r + ',' + g + ',' + b + ',' + a + ')';
        }
      );
...
TSR.TextColorAddOn.colors = TSR.TextColorAddOn.getColorObj(
  TSR.TextColorAddOn.colors
);
...
TSR.TextColorAddOn.frameH = String(
  TSR.Parameters['Sheet Horizontal Frame Size']
);
...
Bitmap.prototype.TextGradientFill = function (x) {
  var y = this._ColorY;
  var gradient = this._context.createLinearGradient(
    x,
    y,
    x,
    y + this._ColorHeight
  ...
  switch (code) {
    ...
    case 'I':
      this.processDrawIcon(
        Window_Base.prototype.obtainEscapeParam(textState),
        textState
      );
      break;
    case '{':
...
Bitmap.prototype.drawIcon = function (
  iconIndex,
  x,
  y,
  iconwidth,
...
Bitmap.prototype.obtainColorsParam = function (textState) {
  this.loadWindowskin();
  var arr = /^\[\d+,*\s*o*\d*\s*w*\d*,*\s*o*\d*\s*w*\d*\]/.exec(
  ...
  if (arr) {
    ...
    if (
      arr[0].includes(',') &&
      arr[0].slice(arr[0].indexOf(',') + 1).includes(',')
    ) {
      if (arr[0].includes('o')) {
        let CG1 = (this.textColor = this.setTextColor(
          parseInt(arr[0].slice(1))
        ));
        let CG2 = (this.textColor = this.setTextColor(
          parseInt(arr[0].slice(arr[0].indexOf(',') + 1))
        ));
        let OC = (this.textColor = this.setTextColor(
          parseInt(arr[0].slice(arr[0].indexOf('o') + 1))
        ));
        this.setTextGradientCT([CG1, CG2]);
        this.outlineColor = TSR.TextColorAddOn.OCop
          ? this.setOCop(OC, 0.5)
          : OC;
        if (arr[0].includes('w')) {
